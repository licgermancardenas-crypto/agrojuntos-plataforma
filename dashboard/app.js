...
/* ----------------------------------------------------------- buscador -- */
/* Un solo campo para las cuatro cosas que alguien busca por su nombre: una
   empresa, una provincia, una región o un territorio. Antes había que saber de
   antemano en qué módulo vivía cada una.

   El índice sale entero de `empresas.json`, que ya trae los cuatro catálogos y
   la correspondencia entre ellos. Se carga la primera vez que se abre el
   buscador y no antes: son dos megas, y no se le cobran a quien no busca.

   Nada se indexa dos veces ni se inventa: las provincias se emparejan con su
   departamento leyendo las filas, porque hay provincias homónimas en regiones
   distintas y «Huamalíes» sin más no lleva a ningún sitio. */
var BUSCA = null;
...
function construirIndice(E) {
  ...
  E.deps.forEach(function (n) {
    items.push({ t: "region", n: n, sub: "región", k: slugU(n) });
  });
...
function indice() {
  ...
  if (!BUSCA_CARGANDO) {
    BUSCA_CARGANDO = cargar("empresas").then(function (E) {
...
function destinoDe(x) {
  ...
  if (x.t === "region") return "#departamentos";
  if (x.t === "provincia") return "/mapa#prov=" + x.k;
  return "/mapa#ter=" + x.rank;
}

/* El departamento no tiene dirección propia dentro del sitio: la vista lo elige
   con un desplegable. Se navega y después se mueve el desplegable, encadenando
   a la misma promesa de datos que usa la vista, de modo que cuando esto corre el
   desplegable ya está lleno. */
function irADepartamento(k) {
  location.hash = "#departamentos";
...
(function armarBuscador() {
  ...
  function elegir(i) {
    ...
    if (!x) return;
    cerrar();
    if (x.t === "region") { irADepartamento(x.k); return; }
    var d = destinoDe(x);
    if (d.indexOf("/mapa") === 0) window.location.href = d;
